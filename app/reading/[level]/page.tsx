...
export default function ReadingLevelPage({ params }: { params: Promise<{ level: string }> }) {
  ...
  const currentData = {
    title: `Gün ${activeDay}: Modern Education & Critical Thinking`,
    text: `In day ${activeDay} of your ${level} program, consistency is the ultimate key. When learners shift their focus from passive memorization to active engagement, fluency accelerates rapidly.`,
    questions: Array.from({ length: 10 }, (_, i) => ({
      id: i + 1,
      question: `Sual ${i + 1}: Mətnin əsas ideyası nədir?`,
      options: ["A) Active engagement", "B) Passive learning", "C) Temporary studying"],
      correct: 0
    }))
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="p-8 bg-slate-900 rounded-3xl border border-white/10 space-y-6 shadow-2xl">
              ...
              <p className="text-gray-300 text-sm md:text-base leading-relaxed">{currentData.text}</p>
            </div>

            <div className="p-8 bg-slate-900 rounded-3xl border border-white/10 space-y-8 shadow-2xl">
              <h3 className="text-xl font-bold">10 Suallıq Test</h3>
              ...
              {!isSubmitted ? (
                <button
                  ...
                  className="w-full py-4 rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-black text-sm shadow-xl"
                >
                  Cavabları Yoxla
                </button>
              ) : (
